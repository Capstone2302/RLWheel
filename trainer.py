...
EpisodeStep = namedtuple(
    "EpisodeStep", field_names=["observation", "action", "PID_action"]
)
...
def iterate_batches(env, net, batch_size):
    ...
    while True:
        ...
        episode_steps.append(
            EpisodeStep(observation=obs, action=action, PID_action=[PID_action])
        )
...
if __name__ == "__main__":
    ...
    signal.signal(
        signal.SIGINT, lambda signum, frame: handle_interrupt(folderName, net)
    )
